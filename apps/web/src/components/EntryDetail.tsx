...
export function EntryDetail() {
    ...
    // Keyboard navigation and mode toggling
    useEffect(() => {
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            if (e.key === "Escape") {
                e.preventDefault();
                // Stop propagation so AppShell doesn't catch it immediately if we're just exiting reading mode
                e.stopPropagation();
                if (isReadingMode) {
            ...
            } else if (isReadingMode && (e.key === "ArrowLeft" || e.key === "ArrowRight")) {
                e.preventDefault();
                const currentIndex = filteredEntries.findIndex(e => e.id === selectedId);
                if (currentIndex === -1) return;

                if (e.key === "ArrowLeft" && currentIndex > 0) {
                    select(filteredEntries[currentIndex - 1].id);
    ...
    }, [entry, isReadingMode, filteredEntries, selectedId, select]);
